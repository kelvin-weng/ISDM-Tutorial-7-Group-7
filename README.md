...
* A booking is associated to only one Travel Company and only one Accomodation Company
* Bookings are saved on the Customer but are made by the Relationship Manager on a call with consent
* Outbound Calls are automatically dialled for RMs by the System
* Scripts and Target Lists are generated by the System for Outbound RM calls
