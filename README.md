...
7. Call centre manager
    ...
    Suprevisor who is in charge of running and operating the call management centre
...
We started the project by assigning different roles to each group members to ensure that we are able to complete the project within the alloted time frame. We broke down the roles based on every members strengths and weaknesses.
...
Initially, we experienced issues as we were confused on how to create the the use case models since there are many versions that can be created. We decided to create two use case models showcasing the current call management system and another one for the proposed system that is going to be developed. 
...
A collaboration diagram is used to showcase the relationships and interactiosn between objects in Unified Modelling Language (UML). It defines the role of each object.
